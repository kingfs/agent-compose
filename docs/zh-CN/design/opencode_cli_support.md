...
- Go 控制面：`internal/app/service.go` 的 `normalizeAgentKind`、
  `internal/agent/definition.go` 的 `normalizeAgentDefinition`、
...
1. 增加 provider 归一化。
   ...
   - `internal/app/service.go`：更新 `normalizeAgentKind`。
   - `internal/agent/definition.go`：provider 白名单允许
     `opencode`。
