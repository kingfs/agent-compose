...
Primary files: `internal/app/service.go`, `internal/exec/executor.go`,
...
| `internal/exec/executor.go` | `AgentDefinitionID` on `ExecuteAgentRequest`; inject `configDB` into `Executor` |
| `internal/loader/manager.go` | Pass agent definition id into agent execution |
| `internal/app/run_handler.go` | Pass `ManagedAgentID` into agent execution |
| `internal/exec/system_prompt_test.go` | **new** — host resolution, fixed-path write/remove tests |
