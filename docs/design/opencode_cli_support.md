...
- Go control plane: `normalizeAgentKind` in `internal/app/service.go`,
  `normalizeAgentDefinition` in `internal/agent/definition.go`, loader
...
1. Add provider normalization.
   ...
   - `internal/app/service.go`: update `normalizeAgentKind`.
   - `internal/agent/definition.go`: allow `opencode` in the provider
     whitelist.
