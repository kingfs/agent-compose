...
func testAgentDefinitionValidationAndProtoMapping(t *testing.T) {
	...
	service := &Service{
		...
		configDB: store,
		store:    NewStoreForConfig(&appconfig.Config{SessionRoot: t.TempDir(), RuntimeDriver: driverpkg.RuntimeDriverBoxlite, DefaultImage: "guest:latest"}),
	}
...
func TestAgentSessionMessageUsesDefinitionProviderEnvForFacade(t *testing.T) {
	...
	if env["ANTHROPIC_BASE_URL"] != "http://agent-compose.test/agent-compose/session/"+sessionID+"/llm/anthropic" {
