...
func TestJupyterConnectTargetUsesGuestHostWhenRoutable(t *testing.T) {
	...
	if got := boxlitedriver.JupyterKernelspecsURL(toDriverProxyState(proxyState)); got != "http://agent-compose-session-1:8888/agent-compose/session/session-1/api/kernelspecs?token=secret+token" {
...
func TestWaitForJupyterProxyUsesGuestHostTarget(t *testing.T) {
	...
	defer cancel()
	err := boxlitedriver.WaitForJupyterProxy(ctx, toDriverProxyState(ProxyState{
		ProxyPath: "/agent-compose/session/session-1/lab",
		GuestHost: "localhost",
		HostPort:  1,
