...
func TestConfigStoreReplaceLoaderTriggersStoresMillisecondScheduleTimes(t *testing.T) {
	...
	if nextFireAtRaw < sqlitestore.StoredUnixMillisecondThreshold() {
...
func TestConfigStoreCreateLoaderRunStoresMillisecondStartedAt(t *testing.T) {
	...
	if startedAtRaw < sqlitestore.StoredUnixMillisecondThreshold() {
		t.Fatalf("expected millisecond started_at, got %d", startedAtRaw)
	}
	if completedAtRaw < sqlitestore.StoredUnixMillisecondThreshold() {
...
func TestConfigStoreAddLoaderEventStoresMillisecondCreatedAt(t *testing.T) {
	...
	if createdAtRaw < sqlitestore.StoredUnixMillisecondThreshold() {
...
func TestEnsureLoaderSchemaMigratesLegacyRunAndEventTimestamps(t *testing.T) {
	...
	if err := store.EnsureLoaderSchemaForTest(); err != nil {
...
func testLoaderRunHostAgentStopsSessionAfterExecution(t *testing.T) {
	...
	manager := &LoaderManager{
		...
		configDB: configDB,
		driver:   driver,
		executor: NewExecutorForTest(config, store, nil, fixedRuntimeProvider{runtime: runtime}, nil),
		engine:   &QJSLoaderEngine{},
		running:  map[string]int{},
	}
	loader := createTestLoader(t, ctx, configDB)
...
func TestLoaderManagerProjectAgentRunnerCarriesImageBackend(t *testing.T) {
	...
	runner := manager.projectAgentRunnerComponent()
	if runner == nil {
		t.Fatalf("projectAgentRunnerComponent returned nil")
	}
	if _, _, err := runner.RunProjectAgent(context.Background(), &agentcomposev2.RunAgentRequest{}); err == nil {
		t.Fatalf("default project agent runner returned nil error")
	}
}
...
func newTestLoaderCommandManager(t *testing.T, ctx context.Context) (*LoaderManager, *fakeLoaderAgentRuntime, *fakeSessionDriver, Loader) {
	...
	manager := &LoaderManager{
		...
		configDB: configDB,
		driver:   driver,
		executor: NewExecutorForTest(config, store, configDB, fixedRuntimeProvider{runtime: runtime}, nil),
		engine:   &QJSLoaderEngine{},
		running:  map[string]int{},
	}
	loader := createTestLoader(t, ctx, configDB)
...
func newTestConfigStore(t *testing.T) *sqlitestore.ConfigStore {
	t.Helper()
	...
	store := sqlitestore.NewConfigStoreForDB(db)
	if err := store.InitSchemaForTest(); err != nil {
		_ = db.Close()
...
func newTestLLMClient(t *testing.T, configDB LLMConfigStore, text string) *LLMClient {
	t.Helper()
...
func (d *fakeSessionDriver) StopSessionVM(_ context.Context, session *Session) error {
	if session != nil {
...
func createTestLoader(t *testing.T, ctx context.Context, store *sqlitestore.ConfigStore) Loader {
	t.Helper()
