...
type LoaderHost interface {
	...
	PublishEvent(ctx context.Context, topic string, payloadJSON string) (events.TopicEventRecord, error)
	Agent(ctx context.Context, prompt string, request AgentRequest) (AgentResult, error)
	Command(ctx context.Context, request CommandRequest) (CommandResult, error)
	LLM(ctx context.Context, prompt string, request LLMRequest) (LLMResult, error)
...
func (e *QJSLoaderEngine) execute(ctx context.Context, request LoaderExecutionRequest, host LoaderHost, validateOnly bool) (LoaderExecutionResult, error) {
	...
	if runtimeName != RuntimeScheduler {
...
func (e *QJSLoaderEngine) installRuntime(jsctx *qjs.Context, state *loaderExecutionState) (*qjs.Value, error) {
	...
	setIntervalFn := jsctx.Function(func(call *qjs.This) (*qjs.Value, error) {
		return registerTimer(TriggerKindInterval, call)
	})
	...
	setTimeoutFn := jsctx.Function(func(call *qjs.This) (*qjs.Value, error) {
		return registerTimer(TriggerKindTimeout, call)
	})
	...
	clearIntervalFn := jsctx.Function(func(call *qjs.This) (*qjs.Value, error) {
		if len(call.Args()) > 0 {
			state.unregister(triggerHandle(call.Args()[0]), TriggerKindInterval)
		}
	...
	clearTimeoutFn := jsctx.Function(func(call *qjs.This) (*qjs.Value, error) {
		if len(call.Args()) > 0 {
			state.unregister(triggerHandle(call.Args()[0]), TriggerKindTimeout)
		}
		return jsctx.NewUndefined(), nil
	})
	...
	onFn := jsctx.Function(func(call *qjs.This) (*qjs.Value, error) {
		...
		if id == "" {
			id = TriggerStableID(TriggerKindEvent, topic, 0, callback.String(), len(state.registrations))
			autoID = true
		}
		trigger := Trigger{
			ID:       id,
	...
	cronFn := jsctx.Function(func(call *qjs.This) (*qjs.Value, error) {
		...
		if id == "" {
			id = TriggerStableID(TriggerKindCron, expr, 0, callback.String(), len(state.registrations))
			autoID = true
		}
		trigger := Trigger{
			ID:       id,
	...
	agentFn := jsctx.Function(func(call *qjs.This) (*qjs.Value, error) {
		...
		options, err := parseAgentRequest(args)
		if err != nil {
			return nil, err
		}
	...
	llmFn := jsctx.Function(func(call *qjs.This) (*qjs.Value, error) {
		...
		options, err := parseLLMRequest(args)
		if err != nil {
			return nil, err
		}
		var outputSchemaValue *qjs.Value
...
func (e *QJSLoaderEngine) executeRequestedHandler(jsctx *qjs.Context, state *loaderExecutionState, trigger *Trigger, payload *qjs.Value) (*qjs.Value, error) {
	global := jsctx.Global()
...
func (s *loaderExecutionState) register(trigger Trigger, callback *qjs.Value) error {
...
func (s *loaderExecutionState) triggers() []Trigger {
	items := make([]Trigger, 0, len(s.registrations))
	for _, item := range s.registrations {
...
func parseCronRegistration(args []*qjs.Value) (string, string, *qjs.Value, string, bool, error) {
	...
	specJSON, err := CronSpecJSON(expr, timezone)
	if err != nil {
		return "", "", nil, "", false, err
	}
...
func parseAgentRequest(args []*qjs.Value) (AgentRequest, error) {
	...
	if err != nil {
		return AgentRequest{}, fmt.Errorf("decode scheduler.agent options: %w", err)
	}
	request.Agent = NormalizeAgentKind(loaderStringOption(options, "agent"))
	request.SessionPolicy = NormalizeSessionPolicy(loaderStringOption(options, "sessionPolicy", "session_policy"))
	request.Timeout, err = loaderDurationOption(options, "timeout", "agentTimeout", "agent_timeout")
	if err != nil {
		return AgentRequest{}, fmt.Errorf("decode scheduler.agent timeout: %w", err)
	}
...
func parseLoaderExecRequest(args []*qjs.Value) (CommandRequest, error) {
	if len(args) != 1 || args[0] == nil || args[0].IsUndefined() || args[0].IsNull() || !args[0].IsObject() || args[0].IsArray() {
		return CommandRequest{}, fmt.Errorf("scheduler.exec requires a request object")
	}
	options, err := qjs.ToGoValue[map[string]any](args[0])
	if err != nil {
		return CommandRequest{}, fmt.Errorf("decode scheduler.exec request: %w", err)
	}
	...
	if strings.TrimSpace(request.Command) == "" {
		return CommandRequest{}, fmt.Errorf("scheduler.exec requires a non-empty command")
	}
	request.Args, err = loaderStringArrayOption(options, "args")
	if err != nil {
		return CommandRequest{}, fmt.Errorf("decode scheduler.exec args: %w", err)
	}
	return request, nil
}

func parseLoaderShellRequest(args []*qjs.Value) (CommandRequest, error) {
	if len(args) == 0 || args[0] == nil || args[0].IsUndefined() || args[0].IsNull() {
		return CommandRequest{}, fmt.Errorf("scheduler.shell requires a script")
	}
	if len(args) > 2 {
		return CommandRequest{}, fmt.Errorf("scheduler.shell accepts a script and optional options object")
	}
	script := args[0].String()
	if strings.TrimSpace(script) == "" {
		return CommandRequest{}, fmt.Errorf("scheduler.shell requires a non-empty script")
	}
	options := map[string]any{}
	if len(args) > 1 && args[1] != nil && !args[1].IsUndefined() && !args[1].IsNull() {
		if !args[1].IsObject() || args[1].IsArray() {
			return CommandRequest{}, fmt.Errorf("scheduler.shell options must be an object")
		}
		decoded, err := qjs.ToGoValue[map[string]any](args[1])
		if err != nil {
			return CommandRequest{}, fmt.Errorf("decode scheduler.shell options: %w", err)
		}
		options = decoded
	}
...
func loaderCommandRequestFromOptions(options map[string]any, apiName string) (CommandRequest, error) {
	var err error
	request := CommandRequest{
		Cwd:           loaderStringOption(options, "cwd"),
		SessionPolicy: NormalizeSessionPolicy(loaderStringOption(options, "sessionPolicy", "session_policy")),
	...
	if err != nil {
		return CommandRequest{}, fmt.Errorf("decode %s env: %w", apiName, err)
	}
	request.TimeoutMs, err = loaderInt64Option(options, "timeoutMs", "timeout_ms")
	if err != nil {
		return CommandRequest{}, fmt.Errorf("decode %s timeoutMs: %w", apiName, err)
	}
	request.MaxOutputBytes, err = loaderInt64Option(options, "maxOutputBytes", "max_output_bytes")
	if err != nil {
		return CommandRequest{}, fmt.Errorf("decode %s maxOutputBytes: %w", apiName, err)
	}
	request.SessionEnv, err = loaderSessionEnvOption(options)
	if err != nil {
		return CommandRequest{}, fmt.Errorf("%s", strings.Replace(err.Error(), "scheduler.agent", apiName, 1))
	}
	return request, nil
}

func loaderCommandResultValue(jsctx *qjs.Context, apiName string, response CommandResult) (*qjs.Value, error) {
	data, err := json.Marshal(response)
	if err != nil {
...
func loaderSessionEnvOption(options map[string]any) ([]EnvVar, error) {
...
func loaderSessionEnvItems(value any) ([]EnvVar, error) {
	switch typed := value.(type) {
	case nil:
		return nil, nil
	case map[string]any:
		...
		for _, key := range keys {
			...
			items = append(items, EnvVar{Name: name, Value: envValue, Secret: secret})
		}
		return NormalizeEnvItems(items), nil
	case []any:
		items := make([]EnvVar, 0, len(typed))
...
func parseLLMRequest(args []*qjs.Value) (LLMRequest, error) {
	request := LLMRequest{}
	if len(args) < 2 || args[1] == nil || args[1].IsUndefined() || args[1].IsNull() {
		return request, nil
	}
	options, err := loaderAgentOptionsWithoutSchema(args[1])
	if err != nil {
		return LLMRequest{}, fmt.Errorf("decode scheduler.llm options: %w", err)
	}
