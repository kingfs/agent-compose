...
type FacadeTokenStore interface {
	GetLLMFacadeToken(ctx context.Context, rawToken string) (FacadeToken, error)
}

type FacadeSessionStore interface {
	GetSession(ctx context.Context, sessionID string) (*sessiondomain.Session, error)
}

type FacadeHTTPClient interface {
	Do(req *http.Request) (*http.Response, error)
}

type RuntimeTargetResolver func(ctx context.Context, requestedModel, providerID string) (ResolvedTarget, error)

type RuntimeFacade struct {
	Tokens        FacadeTokenStore
	Sessions      FacadeSessionStore
	Client        FacadeHTTPClient
...
func (f RuntimeFacade) HandleResponses(c echo.Context) error {
	return f.Handle(c, protocolbridge.ProtocolOpenAIResponses, APIProtocolResponses)
}

func (f RuntimeFacade) HandleChatCompletions(c echo.Context) error {
	return f.Handle(c, protocolbridge.ProtocolOpenAIChat, APIProtocolChatCompletions)
}

func (f RuntimeFacade) HandleAnthropicMessages(c echo.Context) error {
	return f.Handle(c, protocolbridge.ProtocolAnthropicMessages, APIProtocolMessages)
}

func (f RuntimeFacade) Handle(c echo.Context, inboundProtocol protocolbridge.Protocol, facadeWireAPI string) error {
	sessionID := strings.TrimSpace(c.Param("session_id"))
	rawToken := runtimeLLMFacadeToken(c.Request().Header)
	if sessionID == "" || rawToken == "" || f.Tokens == nil {
		return c.JSON(http.StatusUnauthorized, map[string]string{"error": "llm facade token is required"})
	}
	token, err := f.Tokens.GetLLMFacadeToken(c.Request().Context(), rawToken)
	if err != nil {
	...
	if token.WireAPI != "" && NormalizeWireAPI(token.WireAPI) != NormalizeWireAPI(facadeWireAPI) {
	...
	session, err := f.Sessions.GetSession(c.Request().Context(), sessionID)
	if err != nil {
		return c.JSON(http.StatusForbidden, map[string]string{"error": "session is not available"})
	}
	if session.Summary.VMStatus == sessiondomain.VMStatusStopped || session.Summary.VMStatus == sessiondomain.VMStatusFailed {
	...
	if f.ResolveTarget == nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "llm target resolver is not configured"})
	}
	target, err := f.ResolveTarget(c.Request().Context(), FirstNonEmpty(token.Model, model), token.ProviderID)
	if err != nil {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": err.Error()})
	}
	...
	if inboundProtocol == upstreamProtocol {
		...
		return f.proxyRuntimeLLMTransparent(c, upstreamEndpoint, upstreamBody, target, upstreamProtocol)
	}
	...
	ApplyForwardHeaders(upstreamReq.Header, target.Headers)
	if f.Client == nil {
		return c.JSON(http.StatusBadGateway, map[string]string{"error": "llm http client is not configured"})
	}
	...
	if runtimeLLMResponseShouldFlush(resp.Header) {
		return bridgeRuntimeLLMStreamResponse(c, resp, inboundProtocol, upstreamProtocol, NormalizeProviderType(target.Provider.ProviderType), target.Model.Name)
	}
...
func (f RuntimeFacade) proxyRuntimeLLMTransparent(c echo.Context, upstreamEndpoint string, body []byte, target ResolvedTarget, upstreamProtocol protocolbridge.Protocol) error {
	...
	if resp.StatusCode >= 200 && resp.StatusCode < 300 && RuntimeLLMUseGenericResponsesTextParts(target, upstreamProtocol) {
		if runtimeLLMResponseShouldFlush(resp.Header) {
			return bridgeRuntimeLLMStreamResponse(c, resp, protocolbridge.ProtocolOpenAIResponses, protocolbridge.ProtocolOpenAIResponses, ProviderFamilyOpenAI, target.Model.Name)
		}
		upstreamRespBody, err := io.ReadAll(io.LimitReader(resp.Body, 64<<20))
		if err != nil {
			return c.JSON(http.StatusBadGateway, map[string]string{"error": "read upstream llm response failed"})
...
func RewriteRuntimeLLMRequestForUpstream(body []byte, target ResolvedTarget, upstreamProtocol protocolbridge.Protocol) ([]byte, error) {
...
func RuntimeLLMUseGenericResponsesTextParts(target ResolvedTarget, upstreamProtocol protocolbridge.Protocol) bool {
...
func LLMUpstreamProtocolAndEndpoint(target ResolvedTarget) (protocolbridge.Protocol, string, error) {
...
func EncodeRuntimeLLMUpstreamRequest(inboundProtocol, upstreamProtocol protocolbridge.Protocol, target ResolvedTarget, req *protocolbridge.LLMRequest) ([]byte, error) {
	if inboundProtocol == upstreamProtocol {
...
func EncodeRuntimeLLMClientResponse(inboundProtocol, upstreamProtocol protocolbridge.Protocol, target ResolvedTarget, upstreamBody []byte) ([]byte, error) {
	inboundAdapter, err := llmProtocolAdapter(inboundProtocol)
	if err != nil {
		return nil, err
	}
