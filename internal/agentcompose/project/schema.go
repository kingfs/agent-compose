...
func EnsureSchema(ctx context.Context, db *sql.DB) error {
	...
	if err := ensureManagedResourceColumns(ctx, db); err != nil {
		return err
	}
	return nil
}
...
func ensureManagedResourceColumns(ctx context.Context, db *sql.DB) error {
	agentColumns := []struct {
		name       string
		definition string
	}{
	...
	for _, column := range agentColumns {
		if err := ensureColumn(ctx, db, "agent_definition", column.name, column.definition); err != nil {
	...
	for _, column := range loaderColumns {
		if err := ensureColumn(ctx, db, "loader", column.name, column.definition); err != nil {
