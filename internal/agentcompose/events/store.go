...
func (s *EventStore) ensureEventSchema(ctx context.Context) error {
	...
	for column, definition := range map[string]string{
		...
		if err := configdomain.EnsureColumn(ctx, s.db, "event", column, definition); err != nil {
...
func (s *EventStore) CreateEvent(ctx context.Context, item TopicEventRecord) (TopicEventRecord, error) {
...
func (s *EventStore) GetEvent(ctx context.Context, eventID string) (TopicEventRecord, error) {
...
func (s *EventStore) FindEventByIdempotencyKey(ctx context.Context, topic, key string) (TopicEventRecord, bool, error) {
...
func (s *EventStore) ListPendingEvents(ctx context.Context, limit int) ([]TopicEventRecord, error) {
...
func (s *EventStore) ListEvents(ctx context.Context, filter TopicEventFilter) ([]TopicEventRecord, error) {
...
func (s *EventStore) ListDispatchableEvents(ctx context.Context, now time.Time, limit int) ([]TopicEventRecord, error) {
	if limit <= 0 || limit > 500 {
		limit = 100
	}
...
func (s *EventStore) UpsertEventDelivery(ctx context.Context, delivery EventDelivery) error {
...
func (s *EventStore) AddEventSessionLink(ctx context.Context, link EventSessionLink) error {
...
func (s *EventStore) ListEventDeliveries(ctx context.Context, eventIDs []string) ([]EventDelivery, error) {
...
func (s *EventStore) ListEventSessionLinks(ctx context.Context, eventIDs []string) ([]EventSessionTraceItem, error) {
	seen := map[string]struct{}{}
	ids := make([]string, 0, len(eventIDs))
	for _, id := range eventIDs {
		id = strings.TrimSpace(id)
...
func (s *EventStore) ListEnabledWebhookSourcesForTopic(ctx context.Context, topic string) ([]WebhookSource, error) {
	topic = strings.TrimSpace(topic)
	if topic == "" {
...
func webhookSourceTopicMatches(topic, topicPrefix string) bool {
	return events.WebhookSourceTopicMatches(topic, topicPrefix)
}

func (s *EventStore) ListWebhookSources(ctx context.Context) ([]WebhookSource, error) {
	rows, err := s.db.QueryContext(ctx, `SELECT id, name, enabled, provider, topic_prefix, token_hash, signature_type, signature_secret, body_limit_bytes, created_at, updated_at
...
func (s *EventStore) GetWebhookSource(ctx context.Context, sourceID string) (WebhookSource, bool, error) {
...
func (s *EventStore) UpsertWebhookSource(ctx context.Context, source WebhookSource) (WebhookSource, error) {
