...
const (
	TopicEventSourceWebhook   = eventmodule.TopicEventSourceWebhook
	TopicEventDispatchPending = eventmodule.TopicEventDispatchPending
)

type TopicEventRecord = eventmodule.TopicEventRecord
type TopicEventFilter = eventmodule.TopicEventFilter
type WebhookSource = eventmodule.WebhookSource
type EventDelivery = eventmodule.EventDelivery
type EventSessionTraceItem = eventmodule.EventSessionTraceItem

type Store interface {
	FindEventByIdempotencyKey(ctx context.Context, topic, key string) (TopicEventRecord, bool, error)
	CreateEvent(ctx context.Context, item TopicEventRecord) (TopicEventRecord, error)
	UpdateEventPayload(ctx context.Context, eventID, payloadJSON string) error
	GetEvent(ctx context.Context, eventID string) (TopicEventRecord, error)
	ListEvents(ctx context.Context, filter TopicEventFilter) ([]TopicEventRecord, error)
	ListDescendantEventIDs(ctx context.Context, rootEventID string, limit int) ([]string, error)
	ListEventSessionLinks(ctx context.Context, eventIDs []string) ([]EventSessionTraceItem, error)
	ListEventDeliveries(ctx context.Context, eventIDs []string) ([]EventDelivery, error)
	ListWebhookSources(ctx context.Context) ([]WebhookSource, error)
	GetWebhookSource(ctx context.Context, sourceID string) (WebhookSource, bool, error)
	UpsertWebhookSource(ctx context.Context, source WebhookSource) (WebhookSource, error)
	DeleteWebhookSource(ctx context.Context, sourceID string) error
	ListEnabledWebhookSourcesForTopic(ctx context.Context, topic string) ([]WebhookSource, error)
}
...
func NewHandler(bodyLimitBytes int64, store Store) *Handler {
	return &Handler{bodyLimitBytes: bodyLimitBytes, store: store}
}
...
func RegisterRoutes(app *echo.Echo, service *Handler) {
...
func (s *Handler) handleWebhook(c echo.Context) error {
	...
	if existing, ok, err := s.store.FindEventByIdempotencyKey(c.Request().Context(), topic, idempotencyKey); err != nil {
	...
	created, err := s.store.CreateEvent(c.Request().Context(), TopicEventRecord{
		ID:             eventID,
		Topic:          topic,
	...
	if created.Sequence != 0 {
		...
		if err := s.store.UpdateEventPayload(c.Request().Context(), created.ID, payloadJSON); err != nil {
...
func (s *Handler) handleGetEvent(c echo.Context) error {
	eventID := strings.TrimSpace(c.Param("event_id"))
	item, err := s.store.GetEvent(c.Request().Context(), eventID)
	if err != nil {
...
func (s *Handler) handleListEvents(c echo.Context) error {
	...
	items, err := s.store.ListEvents(c.Request().Context(), TopicEventFilter{
		Topic:         topic,
		CorrelationID: correlationID,
...
func (s *Handler) handleGetEventSessions(c echo.Context) error {
	eventID := strings.TrimSpace(c.Param("event_id"))
	item, err := s.store.GetEvent(c.Request().Context(), eventID)
	if err != nil {
		if strings.Contains(err.Error(), "not found") {
	...
	eventIDs, err := s.store.ListDescendantEventIDs(c.Request().Context(), eventID, 1000)
	if err != nil {
		return c.JSON(http.StatusInternalServerError, map[string]string{"error": "failed to trace event descendants"})
	}
	links, err := s.store.ListEventSessionLinks(c.Request().Context(), eventIDs)
	if err != nil {
...
func (s *Handler) handleGetEventRuns(c echo.Context) error {
	eventID := strings.TrimSpace(c.Param("event_id"))
	item, err := s.store.GetEvent(c.Request().Context(), eventID)
	if err != nil {
		if strings.Contains(err.Error(), "not found") {
	...
	deliveries, err := s.store.ListEventDeliveries(c.Request().Context(), eventIDs)
	if err != nil {
...
func (s *Handler) handleListWebhookSources(c echo.Context) error {
	items, err := s.store.ListWebhookSources(c.Request().Context())
	if err != nil {
...
func (s *Handler) handlePutWebhookSource(c echo.Context) error {
	...
	if existing, ok, err := s.store.GetWebhookSource(c.Request().Context(), sourceID); err != nil {
	...
	source, err := s.store.UpsertWebhookSource(c.Request().Context(), WebhookSource{
		ID:              sourceID,
		Name:            req.Name,
		Enabled:         enabled,
...
func (s *Handler) handleDeleteWebhookSource(c echo.Context) error {
	sourceID := strings.TrimSpace(c.Param("source_id"))
	if sourceID == "" {
		return c.JSON(http.StatusBadRequest, map[string]string{"error": "source_id is required"})
	}
	if err := s.store.DeleteWebhookSource(c.Request().Context(), sourceID); err != nil {
		if strings.Contains(err.Error(), "not found") {
...
func (s *Handler) authorizeWebhookRequest(c echo.Context, topic string) (WebhookSource, int64, bool, error) {
	defaultLimit := s.bodyLimitBytes
	sources, err := s.store.ListEnabledWebhookSourcesForTopic(c.Request().Context(), topic)
	if err != nil {
...
func validateTopicEventName(topic string) error {
	return eventmodule.ValidateTopicEventName(topic)
}

func topicEventPayloadSHA256(payloadJSON string) string {
	return eventmodule.PayloadSHA256(payloadJSON)
}
