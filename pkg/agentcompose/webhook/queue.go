...
func normalizeQueueRule(raw webhookQueueRuleConfig) (webhookQueueRule, error) {
	...
	if topic != "" {
		...
		if err := event.ValidateTopicEventName(pattern); err != nil {
...
func (q *RunQueue) Reserve(event TopicEvent) (*Reservation, bool) {
	if q == nil {
...
func (q *RunQueue) match(event TopicEvent) (string, int) {
	if q == nil {
...
func (r webhookQueueRule) matches(event TopicEvent) bool {
	if r.Match.Topic != "" && !TopicMatches(r.Match.Topic, event.Topic) {
		return false
	}
