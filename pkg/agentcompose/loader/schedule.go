...
func TriggerNextFireAt(now time.Time, trigger LoaderTrigger, fired bool) (time.Time, error) {
	return loaderTriggerNextFireAt(now, trigger, fired)
}
...
func TriggerSource(trigger LoaderTrigger) string {
	return loaderTriggerSource(trigger)
}
...
func NormalizeCronSpecJSON(raw string) (string, error) {
	return normalizeLoaderCronSpecJSON(raw)
}
