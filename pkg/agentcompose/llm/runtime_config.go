...
func ensureSessionLLMFacadeConfig(ctx context.Context, config *appconfig.Config, configDB *Store, session *Session, agent, model, source, runID string) (map[string]string, error) {
...
func ensureSessionAgentRuntimeLLMConfig(ctx context.Context, config *appconfig.Config, configDB *Store, session *Session, agent, model, source, runID string) (agentRuntimeLLMConfig, error) {
...
func ensureSessionCodexLLMFacadeConfig(ctx context.Context, config *appconfig.Config, configDB *Store, session *Session, model, source, runID string) (map[string]string, error) {
...
func ensureSessionClaudeLLMFacadeConfig(ctx context.Context, config *appconfig.Config, configDB *Store, session *Session, model, source, runID string) (map[string]string, error) {
	baseURL := guestRuntimeLLMBaseURL(config, session)
	if strings.TrimSpace(baseURL) == "" {
		return nil, nil
	}
...
func ensureSessionOpenCodeLLMFacadeConfig(ctx context.Context, config *appconfig.Config, configDB *Store, session *Session, model, source, runID string) (map[string]string, error) {
...
func ensureSessionOpenCodeAnthropicFacadeConfig(ctx context.Context, config *appconfig.Config, configDB *Store, session *Session, model, source, runID string) (map[string]string, error) {
	baseURL := guestRuntimeLLMBaseURL(config, session)
	if strings.TrimSpace(baseURL) == "" {
		return nil, nil
	}
...
func ensureSessionOpenCodeOpenAIFacadeEnv(ctx context.Context, config *appconfig.Config, configDB *Store, session *Session, model, source, runID string) (map[string]string, error) {
	target, err := resolveRuntimeLLMTargetWithEnv(ctx, config, configDB, session.Summary.ID, llmProviderFamilyOpenAI, model, "", sessionLLMProviderEnvItems(session))
	if err != nil {
		return nil, err
	}
...
func ensureSessionOpenCodeCustomProviderConfig(ctx context.Context, config *appconfig.Config, configDB *Store, session *Session, providerID, model, source, runID string) (map[string]string, error) {
...
func resolveOpenCodeCustomProviderTarget(ctx context.Context, config *appconfig.Config, configDB *Store, session *Session, providerID, model string) (LLMResolvedTarget, error) {
...
func hasAnthropicProviderKey(ctx context.Context, config *appconfig.Config, configDB *Store) bool {
	configKey := ""
	if config != nil {
...
func sessionLLMProviderEnvItems(session *Session) []EnvVar {
	if session == nil {
		return nil
	}
...
func lookupRuntimeBaseURLEnv(session *Session) string {
	...
	for _, items := range [][]EnvVar{session.ProviderEnvItems, session.RuntimeEnvItems, session.EnvItems} {
