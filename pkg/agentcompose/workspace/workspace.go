...
const GitWorkspaceTempDirName = gitWorkspaceTempDirName

type Config = configsvc.WorkspaceConfig

type ConfigRepository interface {
	GetWorkspaceConfig(ctx context.Context, id string) (Config, error)
}
...
func PrepareSessionWorkspace(ctx context.Context, config *appconfig.Config, configDB ConfigRepository, session *Session) error {
...
func PrepareWorkspaceConfig(ctx context.Context, config *appconfig.Config, session *Session, workspace Config) error {
...
func prepareFileWorkspace(config *appconfig.Config, session *Session, workspace Config) error {
...
func fileWorkspaceContentRoot(config *appconfig.Config, workspace Config) (string, error) {
...
func ValidateFileWorkspaceConfig(config *appconfig.Config, workspaceID, configJSON string) (string, error) {
	return fileWorkspaceContentRoot(config, Config{
...
func OpenFileWorkspaceContent(config *appconfig.Config, workspace Config) (FileWorkspaceContent, error) {
...
func prepareGitWorkspace(ctx context.Context, session *Session, workspace Config) error {
	var cfg GitConfig
...
func HostWorkspaceInitialized(workspaceRoot string) (bool, error) {
	return hostWorkspaceInitialized(workspaceRoot)
}

func cloneGitWorkspaceRoot(ctx context.Context, workspaceRoot, cloneURL string, cfg GitConfig) error {
	tempDir := filepath.Join(workspaceRoot, gitWorkspaceTempDirName)
...
func GitCloneArgs(cloneURL string, cfg GitConfig, clonePath string) []string {
	return gitCloneArgs(cloneURL, cfg, clonePath)
}
...
func GitCommitFetchArgs(commit string) []string {
	return gitCommitFetchArgs(commit)
}
...
func GitDeepenFetchArgs(unshallow bool) []string {
	return gitDeepenFetchArgs(unshallow)
}
...
func ApplyGitCredentials(cloneURL string, cfg GitConfig) string {
	return applyGitCredentials(cloneURL, cfg)
}
