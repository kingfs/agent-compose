...
func (s *Store) CreateEvent(ctx context.Context, item TopicEventRecord) (TopicEventRecord, error) {
...
func (s *Store) GetEvent(ctx context.Context, eventID string) (TopicEventRecord, error) {
...
func (s *Store) FindEventByIdempotencyKey(ctx context.Context, topic, key string) (TopicEventRecord, bool, error) {
...
func (s *Store) ListPendingEvents(ctx context.Context, limit int) ([]TopicEventRecord, error) {
...
func (s *Store) ListEvents(ctx context.Context, filter TopicEventFilter) ([]TopicEventRecord, error) {
...
func (s *Store) MarkEventPublished(ctx context.Context, eventID, claimID string, dispatchedAt time.Time) error {
	eventID = strings.TrimSpace(eventID)
...
func (s *Store) UpdateEventPayload(ctx context.Context, eventID, payloadJSON string) error {
	eventID = strings.TrimSpace(eventID)
	if eventID == "" {
	...
	payloadHash := PayloadSHA256(payloadJSON)
...
func (s *Store) ListDispatchableEvents(ctx context.Context, now time.Time, limit int) ([]TopicEventRecord, error) {
	if limit <= 0 || limit > 500 {
		limit = 100
	}
...
func (s *Store) ClaimEvent(ctx context.Context, eventID, claimID string, now, until time.Time) (bool, error) {
	eventID = strings.TrimSpace(eventID)
	claimID = strings.TrimSpace(claimID)
	if eventID == "" || claimID == "" {
...
func (s *Store) ReleaseEventClaim(ctx context.Context, eventID, claimID, status, lastError string, nextAttemptAt time.Time) error {
	eventID = strings.TrimSpace(eventID)
	claimID = strings.TrimSpace(claimID)
...
func (s *Store) MarkEventNoSubscriber(ctx context.Context, eventID, claimID string, dispatchedAt time.Time) error {
	eventID = strings.TrimSpace(eventID)
	claimID = strings.TrimSpace(claimID)
	if eventID == "" || claimID == "" {
		return fmt.Errorf("event id and claim id are required")
...
func (s *Store) UpsertEventDelivery(ctx context.Context, delivery EventDelivery) error {
...
func (s *Store) AddEventSessionLink(ctx context.Context, link EventSessionLink) error {
...
func (s *Store) ListEventDeliveries(ctx context.Context, eventIDs []string) ([]EventDelivery, error) {
...
func (s *Store) ListEventSessionLinks(ctx context.Context, eventIDs []string) ([]EventSessionTraceItem, error) {
	seen := map[string]struct{}{}
	ids := make([]string, 0, len(eventIDs))
	for _, id := range eventIDs {
		id = strings.TrimSpace(id)
...
func (s *Store) ListDescendantEventIDs(ctx context.Context, rootEventID string, limit int) ([]string, error) {
...
func (s *Store) ListEnabledWebhookSourcesForTopic(ctx context.Context, topic string) ([]WebhookSource, error) {
	topic = strings.TrimSpace(topic)
	if topic == "" {
...
func (s *Store) GetWebhookSource(ctx context.Context, sourceID string) (WebhookSource, bool, error) {
...
func (s *Store) UpsertWebhookSource(ctx context.Context, source WebhookSource) (WebhookSource, error) {
...
func (s *Store) DeleteWebhookSource(ctx context.Context, sourceID string) error {
	sourceID = strings.TrimSpace(sourceID)
	if sourceID == "" {
