...
type AgentDefinition struct {
	...
	EnvItems               []configsvc.EnvVar `json:"env_items,omitempty"`
...
func NormalizeAgentDefinition(item AgentDefinition, assignDefaults bool) (AgentDefinition, error) {
	...
	item.EnvItems = configsvc.NormalizeEnvItems(item.EnvItems)
	return item, nil
}
...
func ToProtoAgentDefinition(item AgentDefinition, workspace *configsvc.WorkspaceConfig, validation AgentValidationResult, current AgentCurrentRunSummary, latest *AgentLatestRunSummary) *agentcomposev1.AgentDefinition {
...
func ToProtoEnvItems(items []configsvc.EnvVar) []*agentcomposev1.SessionEnvVar {
...
func ToProtoAgentWorkFiles(workspaceID string, workspace *configsvc.WorkspaceConfig) *agentcomposev1.AgentWorkFiles {
...
func AgentWorkspaceSummary(workspace configsvc.WorkspaceConfig) string {
	switch strings.ToLower(strings.TrimSpace(workspace.Type)) {
	case "git":
