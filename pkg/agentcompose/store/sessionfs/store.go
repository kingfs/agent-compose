...
func NewStoreWithConfig(config *appconfig.Config) *Store {
...
func (s *Store) Config() *appconfig.Config {
	if s == nil {
		return nil
	}
	return s.config
...
func (s *Store) CreateSession(_ context.Context, title, baseWorkspace, driver, guestImage, workspaceID, triggerSource string, workspace *SessionWorkspace, envItems []SessionEnvVar, tags []SessionTag) (*Session, error) {
	...
	session := &Session{
		Summary: SessionSummary{
			...
			TriggerSource: sessionmodel.NormalizeTriggerSource(triggerSource, tags),
			Driver:        driver,
...
func (s *Store) ListSessions(_ context.Context, options SessionListOptions) (SessionListResult, error) {
	...
	for _, entry := range entries {
		...
		s.hydrateSessionGuestImage(session)
		if !sessionmodel.MatchesListOptions(session, options) {
			continue
		}
	...
	offset, limit := sessionmodel.NormalizeListBounds(options.Offset, options.Limit)
	page := sessionmodel.Paginate(sessions, offset, limit)
...
func (s *Store) loadSession(id string) (*Session, error) {
	...
	session.Summary.TriggerSource = sessionmodel.NormalizeTriggerSource(session.Summary.TriggerSource, session.Summary.Tags)
